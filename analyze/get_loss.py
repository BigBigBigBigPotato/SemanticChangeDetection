...
class drow_fig():
    def __init__(self,log1_path,log2_path=None):
        self.log1_path = log1_path
        self.log2_path = log2_path

    def main_vssm_ad(self,log_path):
        log_path = os.path.join(log_path,'log_rank0.txt')
        iters, cd_loss, c_loss = [], [], []
        sek = []
        f = open(log_path).readlines()
        for i, lines in enumerate(f):
            if "change detection loss" in lines:
                iters.append(float(lines.split("is ")[1].split(",")[0]))
                cd_loss.append(float(lines.split("is ")[2].split(",")[0]))
                c_loss.append(float(lines.split("is ")[3].split('\n')[0]))
            elif "SeK is " in lines:
                sek.append(float(lines.split("SeK is ")[1].split('\n')[0]))
        return iters, cd_loss, c_loss, sek
    def drow_loss_evalmetric(self):
        # 可视化设置
        iters, cd_loss, c_loss, sek = self.main_vssm_ad(self.log1_path)
        iters, cd_loss, c_loss = iters[::10], cd_loss[::10], c_loss[::10]
        print(f"第{sek.index(max(sek))}次sek最高，为{max(sek)}")

        # 变化检测损失曲线
        plt.figure(figsize=(12, 6))
        plt.plot(iters, cd_loss, label='cd_loss', color='b', linewidth=0.8)
        plt.plot(iters, c_loss, label='c_loss', color='r', linewidth=0.8)
        plt.title('Change Detection Loss')
        plt.xlabel('Iterations')
        plt.ylabel('Loss')

        plt.legend()
        plt.savefig(os.path.join(self.log1_path,'Loss.png'))
        plt.show()

        # 变化检测评价指标sek曲线
        plt.figure(figsize=(12, 6))
        x = [i for i in range(len(sek))]

        plt.figure(figsize=(12, 6))
        plt.plot(x, sek, label='sek', color='b', linewidth=1)
        plt.title('Sek')
        plt.xlabel('Iterations')
        plt.ylabel('sek')

        plt.legend()
        plt.savefig(os.path.join(self.log1_path,'Metric.png'))
        plt.show()

    def compare(self):
        sek1, sek2 = self.main_vssm_ad(self.log1_path)[-1], self.main_vssm_ad(self.log2_path)[-1]

        sek_length=max(len(sek1),len(sek2))
        if len(sek1)>len(sek2):
            sek2 += [None] * (sek_length - len(sek2))
        else:
            sek1 += [None] * (sek_length - len(sek1))

        # 两个log的sek评价指标对比曲线
        plt.figure(figsize=(12, 6))
        x = [i for i in range(sek_length)]
        plt.plot(x, sek1, label='changeEX sek 80k', color='b', linewidth=0.8)
        plt.plot(x, sek2, label='changeEX sek 24K', color='r', linewidth=0.8)
        plt.title('comparison')
        plt.xlabel('Iterations')
        plt.ylabel('Sek')

        plt.legend()
        plt.savefig(os.path.join(self.log1_path,'comparison.png'))
        plt.show()
...
if __name__ == "__main__":
    ...
    # main_heat()
    # main_vssm1()
    log1=r"D:\PyCharm\Pycharm Project\MambaCD\saved_models\SECOND_MambaSCD_Small_2025-03-21_09-23-46"
    log2=r"D:\PyCharm\Pycharm Project\MambaCD\saved_models\SECOND_MambaSCD_Small_2025-03-14_08-36-29"
    d = drow_fig(log1,log2)
    d.drow_loss_evalmetric()
