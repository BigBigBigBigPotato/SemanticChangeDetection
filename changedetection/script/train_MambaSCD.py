...
from MambaCD.classification.utils import logger
...
class Trainer(object):
    def __init__(self, args):
        ...
        outdir = os.path.join(args.log_path,args.dataset+'_'+args.model_type+'_'+current_time)
        self.logger = logger.create_logger(output_dir=outdir,name='')


    def training(self):
        self.logger.info('-----Begin Training Model-----')
        best_kc = 0.0
        best_round = []
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if (itera + 1) % 10 == 0:
                self.logger.info(f'iter is {itera + 1}, change detection loss is {ce_loss_cd + lovasz_loss_cd}, classification loss is {(ce_loss_clf_t1 + ce_loss_clf_t2 + lovasz_loss_clf_t1 + lovasz_loss_clf_t2) / 2}')
                if (itera + 1) % 2000 == 0:
        ...
        self.logger.info('The accuracy of the best round is {}'.format(best_round))

    def validation(self):
        self.logger.info('-----starting evaluation-----')
        dataset = SemanticChangeDetectionDatset(self.args.test_dataset_path, self.args.test_data_name_list, 256, None, 'test')
        val_data_loader = DataLoader(dataset, batch_size=10, num_workers=10, drop_last=False)
        torch.cuda.empty_cache()
        ...
        self.logger.info(f'Kappa coefficient rate is {kappa_n0}, F1 is {Fscd}, OA is {acc_meter.avg}, '
...
def main():
    ...
    parser.add_argument('--cfg', type=str, default='/Project/MambaCD/changedetection/configs/vssm1/vssm_small_224.yaml')
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument('--pretrained_weight_path', type=str, default='/Project/MambaCD/pretrained_weight/vssm_small_0229_ckpt_epoch_222.pth')

    parser.add_argument('--dataset', type=str, default='SECOND')
    parser.add_argument('--type', type=str, default='train')
    parser.add_argument('--train_dataset_path', type=str, default='/root/autodl-tmp/SECOND/train')
    parser.add_argument('--train_data_list_path', type=str, default='/root/autodl-tmp/SECOND/train.txt')
    parser.add_argument('--test_dataset_path', type=str, default='/root/autodl-tmp/SECOND/test')
    parser.add_argument('--test_data_list_path', type=str, default='/root/autodl-tmp/SECOND/test.txt')
    ...
    parser.add_argument('--model_type', type=str, default='MambaSCD_Small')
    parser.add_argument('--model_param_path', type=str, default='../saved_models')
    parser.add_argument('--log_path', type=str, default='/Project/MambaCD/saved_models')
