...
class Inference(object):
    def __init__(self, args):
        ...
        self.change_map_T1_saved_path = os.path.join(args.result_saved_path, args.model_type, 'change_map_T1')
        self.change_map_T2_saved_path = os.path.join(args.result_saved_path, args.model_type, 'change_map_T2')
    ...
    def infer(self):
        ...
        with torch.no_grad():
            for itera, data in enumerate(val_data_loader):
                ...
                print("pre_change_imgs:",type(pre_change_imgs),pre_change_imgs.shape)
    ...
    def singal_infer(self,img_one,img_two):
        import cv2
        import torchvision.transforms as transforms
        pre_change_imgs = cv2.imread(img_one)
        post_change_imgs = cv2.imread(img_two)
        name = img_one.split("/")[-1]

        transf = transforms.ToTensor()
        pre_change_imgs = torch.unsqueeze(transf(pre_change_imgs),0).cuda()
        post_change_imgs = torch.unsqueeze(transf(post_change_imgs),0).cuda()
        ...
        change_map_T1 = map_labels_to_colors(np.squeeze(preds_A), ori_label_value_dict=ori_label_value_dict,
                                             target_label_value_dict=target_label_value_dict)
        change_map_T2 = map_labels_to_colors(np.squeeze(preds_B), ori_label_value_dict=ori_label_value_dict,
                                             target_label_value_dict=target_label_value_dict)
        image_name = name
        ...
        print(f'Singal Img Inference stage is done!')


def main():
    parser = argparse.ArgumentParser(description="Inference on SECOND dataset")
    parser.add_argument('--cfg', type=str,default='/Project/MambaCD/changedetection/configs/vssm1/vssm_base_224.yaml')
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument('--test_dataset_path', type=str, default='/Project/MambaCD/test_data')
    parser.add_argument('--test_data_list_path', type=str, default='/Project/MambaCD/test_data/test.txt')
    parser.add_argument('--shuffle', type=bool, default=True)
    parser.add_argument('--test_data_name_list', type=list)
    parser.add_argument('--cuda', type=bool, default=True)
    parser.add_argument('--model_type', type=str, default='MambaSCD_Base')
    parser.add_argument('--result_saved_path', type=str, default='/Project/MambaCD/test_data_result/results')

    parser.add_argument('--resume', type=str,default='/Project/MambaCD/pretrained_weight/MambaSCD_Base_SECOND_SeK_0.2292.pth')
...
def my_main():
    ...
    img_one='/Project/MambaCD/test_data/T1/72-2021.04-cropped.jpg'
    img_two='/Project/MambaCD/test_data/T2/72-2024.07-cropped.jpg'
    infer = Inference(args)
    infer.singal_infer(img_one,img_two)
    # infer.infer()
